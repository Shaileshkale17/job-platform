...
function JobSeekerDashboard() {
  ...
  const [ApplicationsActivity, setApplicationsActivity] = useState([]);
  ...
  useEffect(() => {
    const fetchApplications = async () => {
      try {
        ...
        const response = await axios.get(
          `${import.meta.env.VITE_BACKEND_URL}/application/me`,
          {
            headers: {
    ...
    const fetchApplicationsActivity = async () => {
      try {
        const token = localStorage.getItem("token");
        const response = await axios.get(
          `${import.meta.env.VITE_BACKEND_URL}/application/activity`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
      ...
      <div
        ...
        <div className="h-80">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={ApplicationsActivity}>
              ...
              {ApplicationsData.map((app, index) => (
                <tr
                  ...
                  <td className="px-6 py-4">
                    {app.createdAt
                      ? timeAgo.format(new Date(app.createdAt))
                      : "N/A"}
                  </td>
                  <td className="px-6 py-4">
                    <span
